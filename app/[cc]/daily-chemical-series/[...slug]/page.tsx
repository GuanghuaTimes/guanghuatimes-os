...
async function getPostFromParams(params: PostPageProps["params"]) {
  ...
  return allDailyChemicalSeries.find((item) => item.slugAsParams === slug) ?? null;
}
...
export async function generateStaticParams(): Promise<PostPageProps["params"][]> {
  const ccs = ["cn", "en"];
  return allDailyChemicalSeries.flatMap((item) =>
    ccs.map((cc) => ({
      cc,
...
export default async function DailyChemicalSeriesPostPage({ params }: PostPageProps) {
  const post = await getPostFromParams(params);
  ...
  return (
    <Shell as="article" variant="markdown">
      <Link
        href={`/${params.cc}/daily-chemical-series`}
        className={cn(
